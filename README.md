...
- **Multiple speakers, one voice each** — `--speakers {1,auto,N}` diarizes and assigns a separate TTS voice per speaker: leaving it unset (default) follows `diarization.enabled` in the config; `1` forces diarization off for that run regardless of config; `auto` clusters ECAPA speaker embeddings per sentence (no gated model, runs fully local) and forces diarization on; a fixed count like `3` skips auto-detection, clusters into exactly that many speakers, and also forces diarization on. `--voice-map "SPEAKER_00=Phạm Tuyên,SPEAKER_01=Ngọc Huyền"` names voices explicitly per speaker label; `--voice` seeds the first speaker. Without a `--voice-map` entry, each speaker gets a voice round-robin over `voices.speaker_voices`, gender-matched when `voices.gender_detect: true` guesses the speaker's gender from median F0 (< 165 Hz → male) — this assignment logic is VieNeu-only; other TTS providers give every speaker the single resolved voice unless `--voice-map` is given. Diarization writes `<out>.diarized.segments.json` and `<out>.voices.json` alongside the usual artifacts; `resume_from_segments.py` reuses `voices.json` when resuming a run from a saved segments file. `--speakers auto|N` needs the GPU extras (`uv sync --extra local-gpu`: speechbrain + scipy; runs the ECAPA model on CUDA or CPU). The Mac preset (`local-mac`) does not install them by default.
- **Default voices** — `voices.default_male: "Phạm Tuyên"` and `voices.default_female: "Ngọc Huyền"`; calibrated at 0.227 s/syllable (Phạm Tuyên) with `scripts/calibrate_voice.py`, so both local presets use `fit.sec_per_syllable: 0.23`. `--voice "Thanh Bình"` and the other 18 VieNeu presets remain available (`pipeline/tts_vieneu.py` lists all 20); `voices.preset_genders` maps each preset name to a gender, used to gender-match `voices.speaker_voices`. Some VieNeu preset clips render with no leading silence and can clip the first consonant (observed with `Thanh Bình`); `Phạm Tuyên` has a natural ~100–200 ms lead-in, hence the default.
- **Diarization config** — `diarization.enabled` (off by default; followed only when `--speakers` is left unset — `1` forces it off and `auto`/a fixed count forces it on for that run regardless), `diarization.backend: ecapa|pyannote` (`ecapa` needs no token; `pyannote` is gated and needs `diarization.hf_token_env` naming an env var that holds a Hugging Face token), `diarization.max_speakers: 4`, `diarization.threshold: 0.65` (cosine-distance clustering cut, `ecapa` backend), plus `diarization.model` / `diarization.pyannote_model` / `diarization.device`. `diarization.min_cluster_segments: 3` and `diarization.min_cluster_seconds: 3.0` absorb spurious speaker clusters: a handful of very short crops sometimes produce noisy enough ECAPA embeddings that clustering splits them into their own fake speaker — below either threshold the cluster is folded into the nearest large one instead of getting its own voice.
- **Splitting over-long ASR sentences** — `transcription.max_sentence_seconds` (`0` = off, `12` in both local presets) splits run-on ASR sentences longer than this at clause punctuation or the largest word gap, applied after merging; `transcription.min_piece_seconds` (`2.5`) is the minimum length of a split-off piece; `transcription.long_gap_seconds` (`2.0`) always cuts at a silence at least this long inside one sentence regardless of the resulting piece length, catching long mid-sentence pauses that clause-punctuation splitting alone would miss.
- **Pipelined translate + TTS** — `fit.pipelined: true` (both local presets) overlaps translating batch N+1 with shortening/synthesizing batch N on a different device; `translation.parallel_batches` sets how many batches are in flight at once (see "Concurrent translation batches" below).
- **Gentle slow-down for under-filled units** — `fit.min_fill` / `fit.min_tempo`: off by default (`min_fill: 0.0`), both local presets set `min_fill: 0.6`. A unit whose synthesized speech fills less than that share of its slot is slowed via ffmpeg atempo (≥ `min_tempo`, default `0.85`, pitch kept) before the overrun is computed, instead of leaving a long silent tail.
...
- **Keep Ollama warm** — set `OLLAMA_KEEP_ALIVE=30m` when running `ollama serve`; otherwise Ollama unloads the model after 5 minutes idle and the next request (e.g. `gemma4:31b`, ~19 GB) pays a cold reload — measured 20–40 s — before it starts serving again.
...
- **Concurrent translation batches** — set `translation.parallel_batches` above 1 (the GPU preset uses `2`) to translate several batches at once, but only after starting `ollama serve` with a matching `OLLAMA_NUM_PARALLEL` so it can actually serve that many requests concurrently.
...
#### Measured (single runs, not benchmarks)

On a Vast.ai box with 2× RTX 3090 (`config/local_gpu.yaml`, `translation.batch_size: 16`):

| Run | Total | Breakdown |
|-----|-------|-----------|
| DeepMind lecture, 21 min (1267 s source) | 4 m 11 s pipeline + ~2 min ffmpeg merge (outside the tracker) | ASR 1 m 37 s · translate+TTS pipelined 2 m 30 s · 164 units, fill median 0.84, longest unit 11.9 s after splitting (was 163 s before `max_sentence_seconds`) |
| Two-person interview, 134 s, `--speakers auto` | 52 s | 2 speakers detected, correct genders from F0 |
