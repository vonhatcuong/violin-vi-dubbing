...
- **Multiple speakers, one voice each** — `--speakers {1,auto,N}` diarizes and assigns a separate TTS voice per speaker: `1` (default) leaves diarization off; `auto` clusters ECAPA speaker embeddings per sentence (no gated model, runs fully local); a fixed count like `3` skips auto-detection and clusters into exactly that many speakers. `--voice-map "SPEAKER_00=Phạm Tuyên,SPEAKER_01=Ngọc Huyền"` names voices explicitly per speaker label; `--voice` seeds the first speaker. Without a `--voice-map` entry, each speaker gets a voice round-robin over `voices.speaker_voices`, gender-matched when `voices.gender_detect: true` guesses the speaker's gender from median F0 (< 165 Hz → male) — this assignment logic is VieNeu-only; other TTS providers give every speaker the single resolved voice unless `--voice-map` is given. Diarization writes `<out>.diarized.segments.json` and `<out>.voices.json` alongside the usual artifacts; `resume_from_segments.py` reuses `voices.json` when resuming a run from a saved segments file.
